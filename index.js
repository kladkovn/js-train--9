...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const flatArr = arr.flat();  // За допомогою методу flat() вирівнюємо масив.
  const mapArr = flatArr.map(x => x*x);
  // console.log( mapArr);  // За допомогою методу map() підносимо кожен елемент масиву до квадрату.
  const result = mapArr.filter(mapArr => mapArr >= threshold);  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
...
function insertElementAt(arr, index, element) {
  if(!Array.isArray(arr)){
    return ('Вхідний параметр має бути масивом');}
  const newArr = arr.splice(index, 1, element);  // Використовуємо метод splice для вставки елемента на позицію index);
    // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
    // Використовуємо метод splice для вставки елемента на позицію index
  return (newArr, arr);  // Повертаємо масив з вставленим елементом
}
...
function flattenArray(arr) {
  ...
  const flatArr = arr.flat(Infinity);  // Використовуємо метод flat з параметром Infinity, щоб "випрямити" масив на будь-яку глибину вкладеності
  return flatArr;  // Повертаємо "випрямлений" масив
}
